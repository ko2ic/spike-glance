...
class MyGlanceAppWidgetConfigure : ComponentActivity() {
    ...
    private fun updateWidgetIdAndFinishIfNotWidgetRun() {
        ...
        if (extras != null) {
            ...
            lifecycleScope.launch {
                val glanceAppWidgetManager = GlanceAppWidgetManager(this@MyGlanceAppWidgetConfigure)
                val glanceId: GlanceId = glanceAppWidgetManager.getGlanceIdBy(appWidgetId)
                updateAppWidgetState(context = this@MyGlanceAppWidgetConfigure, glanceId = glanceId) {
                }
                val glanceAppWidget: GlanceAppWidget = MyGlanceAppWidget()
                glanceAppWidget.update(this@MyGlanceAppWidgetConfigure, glanceId)
